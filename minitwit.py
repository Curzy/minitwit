...
def init_db():
    ...
    with closing(connect_db()) as db:
        with app.open_resource('schema.sql') as f:
            db.cursor().executescript("""drop table if exists user;
create table user (
...
def timeline():
    """Shows a users timeline or if no user is logged in it will redirect to the public timeline.
       This timeline shows the users' messages as well as all the messages of followed users."""
    if not g.user:
...
def user_timeline(username):
    ...
    if g.user:
        ...
        follower.who_id = ? and follower.whom_id = ?''', [session['user_id'], profile_user['user_id']], one=True) is not None
    return render_template('timeline.html', messages = query_db('''
...
def unfollow_user(username):
    ...
    if whom_id is None:
        abort(404)
    g.db.execute('delete into follower where who_id = ? and whom_id=?',
                 [session['user_id'], whom_id])
    g.db.commit()
